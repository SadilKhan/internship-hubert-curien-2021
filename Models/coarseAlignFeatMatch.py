...
class CoarseAlign:
    ...
    def setSource(self, Is_org) : 
        ...
        with torch.no_grad() : 
            ...
            self.HMultiScale = []
            for i in range(len(self.scaleList)) : 
                feat = F.normalize(self.net(self.preproc(IsList[i]).unsqueeze(0)))
    ...
    def setTarget(self, It_org) : 
        with torch.no_grad() : 
            ...
            self.ItTensor = self.toTensor(self.It).unsqueeze(0)
            self.featt = F.normalize(self.net(self.preproc(self.It).unsqueeze(0)))
    ...
    def getCoarse(self, Mt) : 
        ...
        with torch.no_grad() :  
            ...
            MtExtend = torch.from_numpy(MtExtend).unsqueeze(0).unsqueeze(0)
            
            MtTensor = F.interpolate(input = MtExtend, size = (self.featt.size()[2], self.featt.size()[3]), mode = 'bilinear')
            MtTensor = (MtTensor > 0.5).type(torch.FloatTensor)
            ...
            ## RANSAC 
            ones = torch.FloatTensor(W1MutualMatch.size(0)).fill_(1)
